...
export const ModifyType = () => {
    ...
    useEffect(() => {
        axios.get(`${API_URL}/api/types`).then(res => setAllTypes(res.data))
    ...
    const handleSubmit = (e) => {
        ...
        axios.put(`${API_URL}/api/types/${modifiedType.id}`, dataToPost)
            ...
            <section className="pageFormulaire">
                <section className="create-form">
                    ...
                    <form action="submit" onSubmit={handleSubmit}>
                        <label>
                            <input type="text" name="name" value={modifiedType.name} placeholder="Nom" onChange={handleChange} maxlength="14" />
                        </label>
                        <label>
                            <input type="text" name="color" maxlength="6" value={modifiedType.color} placeholder="Couleur (hexadecimal)" onChange={handleChange} />
                        </label>
                        <UploadImage setFileName={setFileName} setError={setError} />
                        <p>{error}</p>
                        <button type="submit" className="buttonCreate">Modifier</button>
                    </form>
                </section>
                <section className="previewTypeCreation" >
                    <div className="typeCard" style={modifiedType.color.length === 6 ? { backgroundColor: `#${modifiedType.color}` } : { backgroundColor: "#E6ECF2" }}>
